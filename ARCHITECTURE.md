...
17. **Google Play / Apple App Store release** — in progress. Real Android
    testing became possible for the first time this phase: the Android SDK
    (command-line tools, no Android Studio GUI needed — see
    LOCAL_DEVELOPMENT.md) plus a working emulator were set up from scratch,
    and the app was built and run on Android for the first time in this
    project's history. That surfaced two real, previously undetected bugs —
    exactly the kind of gap that only running on the real target platform
    catches, no amount of web/Windows-target testing would have found
    either:
    - `MainActivity` needed to extend `FlutterFragmentActivity`, not the
      default `FlutterActivity` — `flutter_stripe`'s native PaymentSheet is
      implemented as an Android Fragment and crashes on launch without it.
    - `android/app/src/main/AndroidManifest.xml` never declared
      `android.permission.INTERNET`. This one is subtle: it's invisible
      during ordinary `flutter run` debug testing, because Flutter's own
      project template already puts that permission in a separate
      `android/app/src/debug/AndroidManifest.xml` fragment that only merges
      into *debug* builds — which is also why adding it to the main
      manifest didn't change anything observable in this phase's own debug
      testing (see below, the actual persistent blocker was something
      else). A **release** build never gets that debug-only fragment,
      so without this fix a real release APK would have shipped with zero
      network access — Supabase, Stripe, everything — and nobody would
      have found out until Play Store review or a very confused real user.
      (Caught its own near-miss while fixing this: an earlier edit to
      `android/app/src/debug/AndroidManifest.xml` for the network security
      config below accidentally clobbered that file's own pre-existing
      `INTERNET` declaration — restored it before committing.)

    Full login → booking → Stripe click-through on Android is still blocked
    on the development machine this was built on by a local-environment
    issue, not an app bug: antivirus software (Norton, in this case)
    intercepting HTTPS system-wide in a way Windows itself trusts (it
    installs its own root certificate into Windows' store) but the
    emulator's isolated Android OS correctly rejects, since it has no
    reason to know what that certificate is. This was, in fact, the one
    consistent thread through every login attempt in this phase, including
    before either of the two fixes above — diagnosed conclusively by
    checking what certificate a plain host-side HTTPS request actually
    receives (see LOCAL_DEVELOPMENT.md for the exact PowerShell snippet) —
    it resolved to a certificate explicitly named as "generated by Norton
    Antivirus for SSL/TLS scanning." Two of Norton's UI toggles (Smart
    Firewall entirely, and Auto-Protect for an hour) were tried and neither
    stopped the interception — confirmed still active by re-running the
    same diagnostic afterward — pointing to a lower-level network filter
    driver that likely needs a full restart, not a settings toggle, to
    fully release. Added `android/app/src/debug/res/xml/network_security_config.xml`
    (debug builds only, never release) so the app *can* trust a
    user-installed CA for exactly this scenario once one is actually
    installed — this is the right permanent fix for developing behind any
    TLS-intercepting corporate/AV software, not specific to Norton. With
    both real code bugs above fixed, resolving this one local-machine issue
    (most likely just a restart) should be all that's left before a full
    ...
    see Phase 12's note), then real Google Play and Apple Developer
    accounts, signed release builds, and store listings.
